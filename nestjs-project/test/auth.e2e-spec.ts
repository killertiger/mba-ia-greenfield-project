...
/** Supertest types `body` as `any`; every assertion goes through this shape. */
interface ApiBody {
  id?: string;
  sub?: string;
  email?: string;
...
function body(res: request.Response): ApiBody {
  return res.body as ApiBody;
}

/** The mail service is a private dependency of AuthService. */
...
describe('Auth (e2e)', () => {
  ...
  async function captureConfirmationToken(
    ...
    let capturedToken = '';
    jest
      .spyOn(mailerOf(app.get(AuthService)), 'sendConfirmationEmail')
  ...
  async function capturePasswordResetToken(email: string): Promise<string> {
    let captured = '';
    jest
      .spyOn(mailerOf(app.get(AuthService)), 'sendPasswordResetEmail')
      .mockImplementationOnce((_email, _name, token) => {
        captured = token;
        return Promise.resolve();
      });
